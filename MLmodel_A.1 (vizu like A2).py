...
print("R2_tree_non-opt=",tree.score(X_train, y_train))

predictions_tree = tree.predict(X_test)
print("MAE_tree_non-opt=", mean_absolute_error(y_test, predictions_tree), " and MSE_tree_non-opt=", mean_squared_error(y_test, predictions_tree))

plt.figure()
plt.scatter(X_train[:,0], y_train)
plt.scatter(X_test[:,0], y_test)
plt.scatter(X_test[:,0], predictions_tree, c='r')
plt.title('decision tree non opt')
plt.xlabel('date')
...
plt.scatter(X_test[:,2], predictions_tree, c='r')
plt.title('decision tree non opt')
plt.xlabel('category')
plt.ylabel('waste generation (in tons)')
...
def construct_model(X_train, y_train):
    regression_models = [DecisionTreeRegressor()]  
...
y_pred_df = pd.DataFrame(predictions_tree_op)
...
plt.xlabel('date')
plt.ylabel('waste generation (in tons)')
plt.title('ASE1_Cat0 Decision tree_opt')
plt.show()
...
plt.xlabel('date')
plt.ylabel('waste generation (in tons)')
plt.title('ASE2_Cat0 Decision tree_opt')
plt.legend()
plt.show()
